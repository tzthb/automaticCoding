...
model_lstm_1.add(LSTM(512, return_sequences=True))
model_lstm_1.add(LSTM(512, return_sequences=True))
...
model_lstm_1.add(Dense(num_classes, activation='sigmoid'))
...
# Training
model_lstm_1.fit(X_train, y_train, validation_data=(X_test, y_test), epochs=10, batch_size=256, callbacks=[early_stopping])
print('Model trained.')
...
# Save the model
model_lstm_1.save('rnn_sigmoid.h5')
print('Model saved.')
...
plt.show()
#save heatmap
plt.savefig('heatmap_rnnsigmoid.png')
...
#save model plot
print(plot)
plot.savefig('model_plot_rnnsigmoid.png')
